...
> **📌 实施状态说明**
> - **当前实现**: ✅ 本文档描述的功能已完整实现
> - **核心组件**: `WeekSchedule.vue`, `TaskCard.vue`, `AllDayTaskCard.vue`, `AggregationBlock.vue`
> - **实现版本**: v3.2（项目色+优先级透明度系统）
...
> - ✅ 术语变更: "无时间任务" → "无安排任务"
> - ✅ "延后任务"仅指被snooze的任务
> - ✅ 颜色系统: 项目颜色 + 优先级透明度 (公式: `0.25 + priority × 0.15`)
...
> **更新 (2025-11-26)**: 优化透明度公式，提高基础透明度确保可见性。

| 颜色来源 | 计算方式 | 说明 |
|---------|---------|------|
| 背景色 | 任务所属项目的颜色 | 默认紫色 #667eea (无项目时) |
| 透明度 | `0.5 + (priority × 0.1)` | 优先级0=50%，优先级5=100% |
...
| 2 | 70% (0.7) | 较醒目 |
| 1 | 60% (0.6) | 中等 |
| 0 (最低) | 50% (0.5) | 较淡，但仍清晰可见 |

**视觉增强**:

| 属性 | 值 | 目的 |
|------|-----|------|
| 边框 | 1px solid rgba(0,0,0,0.2) | 增强边界定义 |
| 阴影 | 0 2px 4px rgba(0,0,0,0.15) | 提升层次感 |
| Hover效果 | filter: brightness(1.1) | 悬停时轻微提亮 |
| 文字阴影 | 0 1px 2px rgba(0,0,0,0.2) | 提升文字可读性 |

**样式隔离**:
- 任务卡片使用独立类名 `.schedule-task-card` 避免与全局样式冲突
- 全天任务卡片使用 `.schedule-allday-task-card` 独立样式
- 禁用全局 `hover-gloss` 效果的白色渐变覆盖
