...
*   **核心Agent框架**: **LangGraph** (状态驱动)
    *   **LangGraph**: 用于构建有状态、可循环、多分枝的复杂AI Agent，实现"任务启动仪式"和"可交互复盘"等非线性工作流。
    *   **StateGraph模式**: 使用TypedDict定义状态,节点函数纯粹化,支持条件路由和可视化(Mermaid图)。
    *   ❌ **Agno移除**: 设计文档中提到,但实际项目未使用Agno。
*   **最小化LangChain**: **LangChain Core**
    *   **职责**: 仅用于LLM接口抽象(ChatOpenAI、ChatAnthropic等),不使用LangChain的Chain。
    *   **原则**: "LangGraph优先,LangChain最小化" - 避免过度依赖LangChain工具链。
    *   **LLM工厂模式**: 通过`core/llm_factory.py`统一管理多LLM提供商(OpenAI/Claude/Ollama)。
